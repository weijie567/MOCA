...
- [ ] **AGNT-02**: Agent 基于 LangGraph 状态机编排完整处理流程，至少包含：接收请求、意图识别、加载业务上下文、检索规则证据、生成处理建议、风险判断、审批中断、最终响应 8 个节点
- [ ] **AGNT-03**: Agent 能通过结构化工具调用获取订单、退款单、工单数据
- [ ] **AGNT-04**: Agent 能检索知识库，并在回答中引用具体 doc_id、chunk_id 和规则段落摘要
- [ ] **AGNT-05**: Agent 支持会话级短期记忆，在同一 run/session 的多轮对话中记住 order_id、refund_case_id、ticket_id、已检索证据和上一次处理结论
- [ ] **AGNT-06**: Agent 输出结构化执行轨迹 execution trace，记录经过的图节点、工具调用、证据引用、风险判断和审批状态；不输出模型私有推理链
- [ ] **AGNT-07**: Agent 支持 SSE 流式响应，逐步展示当前阶段，例如"读取订单""检索规则""判断风险""等待审批"
- [ ] **AGNT-08**: Agent 在证据不足时必须拒绝生成确定性结论，并返回"缺少哪些信息 / 建议下一步补充什么"

### RAG & Knowledge Base

- [ ] **RAG-01**: 系统支持导入退款规则、补偿规则、客服 SOP、商家 FAQ 等中文模拟知识文档
...
- [ ] **SAFE-01**: 系统能自动判定动作风险等级，包括低风险、中风险、高风险
- [ ] **SAFE-02**: 高风险动作必须自动触发审批，LangGraph 图执行中断并等待人工决策
- [ ] **SAFE-03**: 审批人可以批准或驳回审批请求
- [ ] **SAFE-04**: 审批通过后，Agent 能根据 approval_request_id 恢复执行
- [ ] **SAFE-05**: 审批驳回后，Agent 必须停止执行高风险动作，并返回驳回原因
- [ ] **SAFE-06**: 每次运行必须产生完整审计日志，可按 run_id 查询和回放
- [ ] **SAFE-07**: 风险阈值、角色权限和审批规则必须通过配置文件管理，不硬编码
- [ ] **SAFE-08**: 所有工具调用必须进行权限校验，防止越权读取或执行

### Infrastructure

- [ ] **INFR-01**: Docker Compose 一键启动核心服务，包括 Postgres、Redis、API，可选启动前端
- [ ] **INFR-02**: 种子脚本生成真实感中文模拟数据，包括订单、退款单、工单、用户、角色、商家和知识库
...
- [ ] **EVAL-01**: golden set 至少包含规则问答、退款排障、补偿建议、审批触发、证据不足五类样本
