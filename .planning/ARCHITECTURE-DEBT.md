...
## 2026-08-11 — Phase 64.4 Plan 02 Task 1 — 字符 chunk budget 与 provider final input 分离 ⚠️修复但验证有缺口
...
- **处理状态**：⚠️ 已完成 assembler 本体并通过 Task 1 精确门禁。新增唯一 `PolicyEmbeddingInputAssembler`，每次结构分组、表格 row/header 重复、sentence/clause/whitespace 与最终 tokenizer-window 分割都用 Plan 01 `EmbeddingTokenCounter` 对完整 envelope 计数；最终 frozen DTO 同时返回 citation/search/exact provider string、hash/count/config fingerprint 与深冻结 provenance。空输入有界返回，envelope-dominant/no-progress/overflow fail closed；没有第二套 tokenizer 或字符预算 fallback。
- **证据**：Phase 64.4 Plan 02 Task 1；`src/rag/policy_embedding_input.py`、`src/rag/chunker.py`、`tests/rag/test_token_aware_chunker.py`、`tests/rag/test_block_chunker.py`、`tests/test_chunker.py`；Wave-0 RED 为缺少权威模块的 3 个 collection error，GREEN 精确门禁为 `23 passed, 1 warning`，全仓 `make lint` 与 `git diff --check` 通过。
- **剩余风险 / 继续入口**：Task 2 仍需补齐 combining Unicode、emoji、URL/number、OCR-like、table oversized cell、overlap/recount 与 tokenizer failure 的 adversarial/rebuild 矩阵。Plan 04 前 production ingestion/dry-run 尚未消费该 DTO，所以当前生产 character path 仍保持不变；必须在 Plan 04 convergence gate 通过后再将本条升级为端到端已修复。
