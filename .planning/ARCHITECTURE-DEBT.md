...
## 2026-08-12 — Phase 64.4 Plan 15 — reviewed claim 只发布 state v2，与 build 的连续 state artifact 契约冲突 🔴待立项
...
- **处理状态**：🔴 待单独 reviewed bounded repair。Plan15 已在第一次 live build 的 state load 阶段 fail closed；没有 candidate build attempt/result、provider construction/request、A-B budget/reservation、selection、authorization或activation。当前唯一新 candidate `64932871-4488-4b8b-b438-a02791a1151f` 保持 building v2/index0、projection `0/0/0`，descriptor/run `f8e190f1-ad0e-4476-9f77-704735e3dc46` 不变。
- **证据**：Phase64.4 Plan15；descriptor commit `38378cc1`、claim artifact commit `3d01f29c`；`scripts/reindex_policies.py:286`、`scripts/reindex_policies.py:308`；canonical run 根只有 descriptor、build-budget manifest与 `states/00000002.json`，build attempts/results 均为 0，canonical A-B仍 `0/2`。
- **剩余风险 / 继续入口**：修复必须先增加真实 claim→build 顺序回归，裁决是依序发布 v1/v2 还是修改 state目录连续性定义，并验证 DB-commit/file-publication crash reconcile；修复后只允许恢复同一 descriptor/candidate，重新 strict-check 两小时 lease、retained parity与source/evidence authority。若 lease过期，不能在未评审情况下续租、rebind或创建第二 candidate。
