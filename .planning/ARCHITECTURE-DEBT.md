...
## 2026-08-13 — Phase 64.5 Plan 02 — DB authority 运行时边界与 crash-spent 预算 ✅已修复验证

- **子系统**：RAG reviewed reindex / canonical A-B / provider execution promotion、authority、reservation、result 与 projection reconciliation。
- **问题现象 / 根因**：Plan 01 只有 append-only schema；尚无 owner 从当前受保护代码与 singleton promotion 签发共享 root，也没有在独立事务中先提交 reservation、用数据库时间做 provider 前复核、把无 result crash 计为已消费 ordinal，或由 DB 单向重建非授权文件 projection。若直接接线，file-backed 旧预算缺陷仍可能被复制 namespace、进程 crash 或并发 retry 放大。
- **影响**：同一 tenant/run/candidate 可能缺少唯一共享 authority；reviewed build 与 canonical A-B 无法共同受一个 root 约束；ordinal 1 crash 后可能被错误退款；非 allowlisted terminal outcome、漂移后的 envelope 或 ordinal 3 可能被误当重试授权；复制的 projection 也可能被误用为 provider authority。
- **处理状态**：✅ 已修复并以串行真实 PostgreSQL 验证。新增 frozen DTO/service/repository：promotion 只接受当前 clean C1 commit/tree 与受保护路径的 exact C0→C1 diff，singleton bytes 不同即拒绝；authority issue、reservation commit 与 dispatch recheck 均先复核 current promotion，并按 promotion→tenant→source/evidence/candidate→authority→reservation/result 锁序使用 `clock_timestamp()`。一个 root 支持两种 purpose；reservation 在独立事务提交后即永久消费，ordinal 2 只允许 committed ordinal-1 的 `provider_unavailable`/`transient_execution_error` 且所有 subject/envelope bytes 不变，ordinal 3 拒绝。result 一 reservation 一 immutable row、request count 不超过 sealed envelope；reconciliation 只从 committed DB graph 以 create-only exact bytes 写 projection，缺失可重建，复制/冲突文件不能签发 authority 或恢复预算。
- **证据**：Phase 64.5 Plan 02；commits `44d5a471`、`7a2180e0`；`src/rag/provider_execution_authority.py`、`src/repositories/provider_execution_authority_repo.py`、`tests/rag/test_provider_execution_authority.py`。最终 gate 为 `make format` PASS、完整 `make lint` PASS、真实 PostgreSQL 专项 `14 passed, 1 warning in 28.97s`；覆盖 ordinal-1 无结果 crash、ordinal-2 并发、ordinal-3、闭集结果、manifest/parity/source/envelope drift、expiry equality、promotion mismatch、copied/missing/corrupt projection 与无退款。
- **剩余风险 / 继续入口**：本计划没有恢复 production provider dispatch，也没有实现 phase-specific checker 对 attestation agent 字符串/角色/evidence bytes 的语义审查；该 checker 与调用方接线由 Plans 03–05 承担，且必须保持 require-current-promotion、commit-before-provider、immediate pre-provider recheck。Plans 06–08 完成 live candidate/A-B/activation drill 前，SC-64.4-5/6 仍不得宣称整体完成。

## 2026-08-13 — Phase 64.5 Plan 03 — reviewed candidate build 已切换为 DB authority，production 仍关闭 ✅已修复验证

- **子系统**：RAG reviewed policy reindex / provider execution authority 接线。
- **问题现象 / 根因**：Phase 64.4 的 reviewed build 仍在候选 artifact namespace 中创建 file-backed attempt/result；即使文件采用 create-only 与 pinned namespace，它仍不能抵抗完整 tree 复制，也无法保证 reservation 在 provider 构造前以独立数据库事务全局提交。known UUID 或复制 projection 若被误作 authority，同样可能绕过真实 root。
- **影响**：provider 成本可能不受数据库全局 ordinal 上限约束；crash 后文件状态与候选 DB cursor 可能分叉；source/parity/lease/config/promotion 在 reservation 后漂移时仍可能错误构造 provider。
- **处理状态**：✅ 已修复验证。reviewed service 现在按 current promotion → deterministic preflight → DB reservation commit → fresh DB-time exact recheck → provider construction/call（事务外）→ candidate/result commit → DB-derived create-only projection 顺序执行；per-document envelope 固定 ordered input hashes/count、batch 10、`ceil(count/10)` calls、SDK retry 0 与一个 outer attempt。`issue-provider-authority` 只从 committed candidate/config/source/parity binding 派生共享 root，不接受 purpose/envelope，不做 provider 工作；文件与 authority UUID 均不能替代 exact DB row。旧 file budget 不再被 reviewed build 消费。
- **证据**：Phase 64.5 Plan 03；`src/rag/policy_reindex.py`、`scripts/reindex_policies.py`、`tests/rag/test_policy_reindex.py`、`tests/rag/test_provider_execution_authority.py`。最终严格 gate：`make format`、完整 `make lint` 均通过；真实 PostgreSQL scoped 两文件 `67 passed, 1 warning in 82.46s`，覆盖 copied projection、promotion/root/drift、crash-spent、conflicting second root、legacy budget 非授权与双 CLI 最早零副作用拒绝；未调用 live provider。
- **剩余风险 / 继续入口**：production `build-next-reviewed` 与 legacy `build-next` 仍在 `_main` parse 后第一 gate 统一返回 `live_provider_execution_disabled`，Plan 03 不授权 live dispatch。Plan 05 完成独立 review/security promotion 前不得打开 reviewed command；legacy command 永久关闭。Plans 06–08 完成真实候选、A/B 与 activation drill 前，SC-64.4-5/6 仍不能作为整阶段完成结论。

## 2026-08-13 — Phase 64.5 Plan 04 — canonical A/B logical query 数遗漏 rewrite provider channel ⚠️修复中

- **子系统**：RAG canonical full-provider A/B / retrieval rewrite / provider request envelope。
- **问题现象 / 根因**：Plan04 以两角色、三格式、18 个基础问题锁定 108 个 query request；但真实 `PolicyRetrievalEngine._retrieve_run` 先调用 original channel，再对每个 deterministic rewrite 调用独立 channel，而 `_retrieve_query_channel` 每次都执行 `embed_query`。sealed Gold 当前有 3 个问题各生成一个 rewrite，所以真实 provider query 上界是126，不是108。
- **影响**：若 reservation 只封 108 query + 16 ingestion batches，真实运行可能在未被 envelope 列举的18个 rewrite provider requests上越界；若直接改成126又违反已锁 Plan04 acceptance，若静默禁用 rewrite则会改变 runtime config与基线语义。
...
## 2026-08-13 — Phase 64.5 canonical A/B schema compatibility 锁死在 migration 031 ⚠️修复但验证有缺口
...
- **处理状态**：⚠️ 已按 WR-01 修复并完成 focused 验证。format parity 与 canonical A/B 均先核对实际 relation/pgvector capability，再要求唯一 DB head 是仓库 Alembic graph 中已知的 schema-floor descendant；029→032 合法 lineage通过，pre-floor、缺 capability与未知 future revision继续 fail closed。既有 `source_drift` 不重跑、不改写、不使用 ordinal 2，也不激活或派发 Plan08。
- **证据**：Phase64.5 Plan07 / WR-01；`scripts/eval_rag_format_parity.py`、`scripts/eval_rag_token_chunk_ab.py`及两份直接测试；最小 RED `2 failed, 1 warning`，GREEN `2 passed`，最终 format/lint PASS、focused `197 passed, 1 warning`。原 terminal仍为 reservation `b0e3a1b2-...`、result `a17f70e7-...`、actual/max `0/142`。
- **剩余风险 / 继续入口**：root尚需独立 full suite与新 exact HEAD code/security re-review；本修复不会追溯性授权或修复已消费 terminal lineage。专用 schema failure result code仍可在后续 phase评估，但不得借此重开本轮 provider预算。
