...
**Primary recommendation:** Use five plans in this order: contracts+persistence, `risk_gate` binding, approval/API manager-scope restore, action-draft validation/projection, then static/focused/eval closure. [VERIFIED: .planning/phases/34-approval-and-actiondraft-boundary-hardening/34-CONTEXT.md D-29]
...
1. **What exact field shape should represent claim verification refs on persisted approval/draft rows?** What we know: `ClaimVerificationBundleV1` exists and carries safe support refs, claim results, blocked claims, route, and verifier policy version. [VERIFIED: src/knowledge/schemas.py] What's unclear: the current bundle does not expose a single stable persisted bundle id/ref in the inspected schema. [VERIFIED: src/knowledge/schemas.py] Recommendation: store a safe JSON summary/ref set in Phase 34 unless implementation introduces a stable bundle ref with tests. [VERIFIED: 34-CONTEXT.md D-02]
2. **Should Phase 34 enable any auto-draft route?** What we know: the contract requires `risk_gate` to own auto-draft routing semantics, and current ActionService safely rejects missing approval as `AUTO_ALLOWED_BINDING_REQUIRED`. [VERIFIED: 34-CONTEXT.md D-16-D-19; VERIFIED: src/actions/service.py] What's unclear: current policy may still require all merchant action drafts to go through approval. [VERIFIED: 34-CONTEXT.md D-17] Recommendation: define the durable auto-allowed binding contract but keep runtime fail-closed unless a plan explicitly proves the full binding path. [VERIFIED: 34-CONTEXT.md D-16-D-19]
3. **How should target merchant be derived from existing business fact refs for non-merchant resource types?** What we know: target merchant must come from scoped `BusinessFactRefV1` / `BusinessFactResultV1` authority or service-approved target merchant derived from those refs. [VERIFIED: 34-CONTEXT.md D-11] What's unclear: `BusinessFactRefV1` fields include resource type/id/version and not a dedicated merchant_id field in the contract snippet. [VERIFIED: src/tools/contracts.py] Recommendation: implement a small service-owned derivation/validation function that fails closed on missing, ambiguous, multi-merchant, or outside-scope targets. [VERIFIED: 34-CONTEXT.md D-13]
