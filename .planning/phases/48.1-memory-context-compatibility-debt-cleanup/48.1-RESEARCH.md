...
| MEM-COMPAT-01 traceability gap | `MEM-COMPAT-01` is referenced by `.planning/ROADMAP.md` and `.planning/STATE.md`, but it is not present in `.planning/REQUIREMENTS.md`. [VERIFIED: rg over .planning/REQUIREMENTS.md .planning/ROADMAP.md .planning/STATE.md] | Planner should include a documentation task only if the phase process requires central requirements traceability; this research does not recommend widening implementation scope beyond the roadmap success criteria. [VERIFIED: .planning/ROADMAP.md] [ASSUMED] |
</phase_requirements>
...
| A3 | The planner may decide whether to add `MEM-COMPAT-01` to `.planning/REQUIREMENTS.md` based on GSD traceability needs. | Phase Requirements | If central requirements must be authoritative before planning, a docs-only task may be required before implementation tasks. |

## Open Questions

1. **Should `MEM-COMPAT-01` be added to `.planning/REQUIREMENTS.md` before planning?** [VERIFIED: rg over requirements/roadmap/state]
   - What we know: Roadmap and state reference `MEM-COMPAT-01`; requirements file does not. [VERIFIED: .planning/ROADMAP.md] [VERIFIED: .planning/STATE.md] [VERIFIED: .planning/REQUIREMENTS.md]
   - What's unclear: Whether this inserted decimal phase requires central requirement registration before plan creation. [ASSUMED]
   - Recommendation: Add a small plan task only if `gsd-plan-checker` requires it; do not broaden code scope for this. [ASSUMED]

2. **Should `ConversationSummary.case_id` be left null, legacy-copied, or supplemented with link metadata?** [VERIFIED: src/db/models.py] [VERIFIED: src/conversation/repository.py]
   - What we know: Summary rows currently store a legacy string `case_id` copied from `ConversationThread.case_id`. [VERIFIED: src/conversation/repository.py] [VERIFIED: src/db/models.py]
   - What's unclear: Product-facing historical display behavior for summaries with multiple linked cases is not specified in Phase 48.1 artifacts. [ASSUMED]
   - Recommendation: Do not add schema columns; tests should verify active relationship logic uses `ThreadCaseLinkRepository` while legacy summary field is not canonical. [VERIFIED: .planning/ROADMAP.md] [ASSUMED]
