...
**Preflight health check (MANDATORY — runs before any scenario):**
...
# 2. Auth endpoint works (validates DB migration)
...
echo "  ✓ Demo user cs_zhang authenticated"

# 3. Manager token works
...
echo "  ✓ Demo user mgr_li authenticated"

# 4. Agent chat endpoint exists
if ! curl -sf -o /dev/null -w "%{http_code}" "$BASE_URL/api/v1/agent/chat" -X POST \
  -H "Authorization: Bearer $(echo $AUTH_RESPONSE | jq -r '.access_token')" \
  -H "Content-Type: application/json" \
  -d '{"query":"ping","thread_id":"preflight"}' 2>/dev/null | grep -qE "^(200|422|500)"; then
  echo "FAIL: Agent chat endpoint not responding"
  echo "  Fix: Check API logs with: docker compose logs api"
  exit 1
fi
echo "  ✓ Agent chat endpoint responding"
...
**Demo output format (MANDATORY for each scenario):**

Each scenario MUST output in this structured format (NOT raw JSON dumps):
```bash
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
echo "Query: [Chinese query text]"
echo "Expected: [what should happen]"
echo ""
echo "Response:"
echo "$RESPONSE" | jq '{intent: .intent, status: .status, risk_level: .risk_level, approval_required: .approval_required, evidence_count: .evidence_count}'
echo ""
echo "Key fields:"
echo "  intent: $(echo $RESPONSE | jq -r '.intent')"
echo "  approval_required: $(echo $RESPONSE | jq -r '.approval_required')"
echo ""
```
...
- Tech stack one-liner: Python 3.12, FastAPI, LangGraph, PostgreSQL + pgvector, Redis, React + Vite
...
frontend/           # React + Vite chat + approval UI
