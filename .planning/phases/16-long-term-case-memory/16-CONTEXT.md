...
- **D-30:** Source identity fallback includes stable source refs such as `conversation_message_id`, `tool_result_id`, `agent_run_id`, and `business_object_id`. Unknown arbitrary JSON keys must not participate in identity matching.
