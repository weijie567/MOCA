...
| Draft idempotency key construction | API / Backend | Database / Storage | Trusted service/server code must construct the key; the DB unique constraint must be `unique (tenant_id, idempotency_key)` per the normative contract, with service/repository exact-binding checks enforcing safe reuse. [VERIFIED: docs/contract-spec.md Section 18.3; .planning/phases/14-demo-action-executor-boundary/14-CONTEXT.md; src/db/models.py] |
...
| Cross-tenant idempotency collision | Tampering | Embed tenant id in the service-built key, enforce `unique (tenant_id, idempotency_key)`, and keep tenant comparison as defense-in-depth in repository reuse checks. [VERIFIED: docs/contract-spec.md Section 18.3; .planning/phases/14-demo-action-executor-boundary/14-CONTEXT.md; src/db/models.py] |
