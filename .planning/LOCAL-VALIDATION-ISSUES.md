...
## 2026-08-13 — Phase 64.5 Plan 05 reviewed route RED 与 C0 重放时点

**问题现象 / 如何检测**
最小 RED `UV_CACHE_DIR=/tmp/uv-cache uv run pytest tests/rag/test_policy_reindex.py::test_production_dispatch_routes_reviewed_build_to_db_promotion_guard -q` 按预期失败：`build-next-reviewed` 仍被旧全局 hard-disable 截断，未进入 DB promotion guard。完成两条 reviewed route 接线后，Task 1 四文件专项为 `191 passed, 1 warning`；但按 plan 把 `review-attestations --require-current-protected-base` 放在未提交 C1 protected diff 之后执行，checker 以 `protected_code_dirty` 正确拒绝。

**关键证据 / 当前判断 / 根因**
C0 三份 root-owned 证据在编辑前已经 strict-load 通过，SHA-256 分别为 gate `c491e66e79691749643a76bebe66e6378a216b89302421eb223d5554011bdd76`、code attestation `2a40322c35cfa20a00d6d89d3d491c5c3a4f8a2769fea4a371648b246fd6486c`、security attestation `3c6b32294dae2572b8aac49f6cef45e2547ea836a1795972839c70eec8fecdb2`。后续拒绝不是 attestation 损坏，而是 `--require-current-protected-base` 的语义只能接受与 C0 等价的 clean protected graph；Plan 05 正在有意制造 C0→C1 protected delta，编辑后再次要求 C0 current 必然冲突。
