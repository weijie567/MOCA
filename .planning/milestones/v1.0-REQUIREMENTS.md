...
- [x] **TOOL-01**: 系统提供订单查询工具 get_order
- [x] **TOOL-02**: 系统提供退款单查询工具 get_refund_case
- [x] **TOOL-03**: 系统提供工单历史查询工具 get_ticket_history
- [x] **TOOL-04**: 系统提供补偿券草稿工具 create_coupon_grant_draft
- [x] **TOOL-05**: 系统提供审批单创建工具 create_approval_request
- [x] **TOOL-06**: 所有工具必须有明确的输入 schema 和输出 schema
- [x] **TOOL-07**: 所有工具调用必须记录 tool_call_id、run_id、tenant_id、user_id、latency_ms、status 和 error_code
- [x] **TOOL-08**: 所有写操作必须支持 idempotency_key，避免重复执行
...
- [x] **INFR-01**: Docker Compose 一键启动核心服务，包括 Postgres、Redis、API；healthcheck 确保启动顺序正确
- [x] **INFR-02**: 种子脚本生成真实感中文模拟数据（80+ 订单、30+ 退款、15+ 知识文档、12+ 用户）
- [x] **INFR-03**: JWT + OAuth2 scopes 实现角色权限控制，至少支持商家运营、平台客服、风险审核员、运营主管、系统管理员
- [x] **INFR-04**: 系统为每次 Agent 执行生成 run_id、trace_id 和 step_id
- [x] **INFR-05**: 系统记录基础可观测性数据，包括 latency_ms、token_usage、cost、error_code、tool_call_status
...
- [x] **DATA-01**: 系统包含 tenants、users、roles、user_roles、merchants 表
- [x] **DATA-02**: 系统包含 orders、refund_cases、tickets 表
- [x] **DATA-03**: 系统包含 policy_documents、policy_chunks 表
- [x] **DATA-04**: 系统包含 agent_runs、agent_steps 表
- [x] **DATA-05**: 系统包含 approval_requests、approval_steps 表
- [x] **DATA-06**: 系统包含 audit_logs、llm_usage_events 表
- [x] **DATA-07**: 所有核心业务表必须包含 tenant_id
- [x] **DATA-08**: MVP 阶段使用应用层 tenant_id 过滤，生产化版本应升级为 PostgreSQL Row-Level Security
...
*Last updated: 2026-05-22 before v1.0 milestone archive — Phase 1 traceability corrected to complete based on shipped roadmap and project state*
