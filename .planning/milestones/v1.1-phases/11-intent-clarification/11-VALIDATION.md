...
- **After every plan wave:** Run `uv run pytest tests/agent/test_nodes/test_classify_intent.py tests/agent/test_nodes/test_receive_request.py tests/agent/test_graph.py tests/agent/test_intent_adapter.py tests/agent/test_intent_routing.py tests/agent/test_required_slots.py tests/agent/test_clarification_gate.py tests/agent/test_intent_manifest.py tests/agent/test_intent_golden_contract.py -q`.
...
| 11-01-02 | 01 | 1 | INTENT-01, CLARIFY-01 | T-11-01 | `receive_request` clears all Phase 11 turn-scoped intent, slot, routing, and clarification fields on every new turn. | unit | `uv run pytest tests/agent/test_nodes/test_receive_request.py -q` | partial | pending |
...
| 11-05-02 | 05 | 5 | INTENT-01, INTENT-02, CLARIFY-01 | T-11-04 | Intent golden cases execute against deterministic pre-router, route, and required-slot helpers, not only manifest counts. | contract/eval | `uv run pytest tests/agent/test_intent_golden_contract.py -q` | no W0 | pending |
...
- [ ] `tests/agent/test_intent_golden_contract.py` - executes `intent-golden.v1` cases against deterministic pre-router, precedence, route, and required-slot helpers.
- [ ] `eval/intent/intent-golden.v1.json`, `eval/intent/coverage-manifest.v1.json`, `eval/intent/m6-statistical-gate.v1.json`, and `eval/intent/intent-consistency.v1.json` - include owner/version/hash fields.
