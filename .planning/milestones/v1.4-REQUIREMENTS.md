# Requirements Archive: v1.4 RAG Production Ingestion + OCR

**Archived:** 2026-06-19
**Status:** SHIPPED

The active `.planning/REQUIREMENTS.md` file was removed after this archive so the next milestone can start with fresh requirements.
...
- Post-dependency gates later resolved the only environment-dependent checks: local `chi_sim` traineddata was installed, the disposable pgvector PostgreSQL live migration round trip ran with `MOCA_TEST_DATABASE_URL`, and the full suite passed with `1136 passed, 9 warnings`.
