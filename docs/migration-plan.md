...
| Phase 9 | Business tool facade | Phase 7 | `src/business_tools/service.py`、contracts、demo adapters；ToolCallContext/ToolResult v2；`ToolRegistry`/`ToolDescriptor`（contract-spec §12.6，工具声明/dispatch/校验单一入口，read/retrieval/write 全量声明） | permission/scope；not_found；timeout；partial_success；invalid_response | read tools 统一走 BusinessToolService；node 不直接访问 repo/tool internals | 可回滚单个 node 调用；不实现写动作 |
