...
| long-term semantic alias | `LongTermMemoryView` 新增 `semantic_kind`，把 legacy `memory_kind` 投影成 `durable_profile_fact` / `merchant_preference` / `merchant_pattern` / `operational_constraint`，不改 DB。 | `src/memory/schemas.py`, `src/memory/repository.py`, `src/agent/context/projectors.py` |
...
目标里的 `LongTermMemory fact` 在实现和文档中应收窄为 durable memory，而不是业务事实系统的替代品。

允许进入 long-term memory 的 durable 语义：

| 建议叫法 | 说明 |
| --- | --- |
| `durable_profile_fact` | 稳定画像事实，例如商家长期服务配置、长期联系人偏好。 |
| `merchant_preference` | 商家明确偏好的处理方式、回复风格、升级路径。 |
| `merchant_pattern` | 经 review 的跨 case 稳定模式，不能指向单个当前订单结论。 |
| `operational_constraint` | 稳定运营约束，例如固定处理窗口、固定审核要求。 |
...
| --- | --- |
| `explicit_user_preference` | 非 PII、非 tombstone、非当前业务状态时可 auto publish long-term。 |
| `explicit_admin_preference` / `human_reviewed` | 非 PII、非 tombstone 时可 auto publish；仍不能替代政策或业务事实证据。 |
| `deterministic_tool_result` | 只有 durable 且不是当前业务对象状态时可 auto publish；若 `source_ref.business_object_type` 缺失，或是 order/refund/refund_case/ticket/logistics/approval/action/coupon/payment，则降级为 `needs_review`。 |
| `confirmed_business_outcome` / `approved_approval_state` | 同上：只允许 durable 非当前状态自动发布；缺失业务对象 metadata 或当前业务对象状态都不能直接发布为 long-term authority。 |
| `llm_candidate` | 默认 `needs_review`。 |
| `semantic_episode_candidate` / `summary_candidate` | 默认 `needs_review`。 |
| `cross_case_pattern_candidate` / `behavior_inference` | 默认 `needs_review`。 |
