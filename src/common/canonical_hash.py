...
_FIXED_MILLISECOND_UTC_RE = re.compile(
    ...
    *,
    ...
    if value.get("schema_version") not in (None, schema_version):
        raise CanonicalHashError(
            f"schema_version {value.get('schema_version')!r} does not match {schema_version!r}"
        )
...
def _normalize_value(value: Any, *, nullable_fields: set[str], path: tuple[str, ...]) -> Any:
    ...
    if isinstance(value, list):
        return [
            _normalize_value(item, nullable_fields=nullable_fields, path=(*path, "[]"))
            for item in value
        ]
    ...
    raise CanonicalHashError(
        f"unsupported canonical JSON value at {_format_path(path)}: {type(value).__name__}"
    )
...
def _validate_field_semantics(value: int | str, *, path: tuple[str, ...]) -> None:
    ...
    if field_name == "amount":
        if not isinstance(value, str) or not _MONEY_AMOUNT_RE.fullmatch(value):
            raise CanonicalHashError(
                f"amount must be a normalized decimal string with fixed 2-digit scale at "
                f"{_format_path(path)}"
            )
        return
    ...
    if field_name.endswith("_at") and isinstance(value, str):
        if not _FIXED_MILLISECOND_UTC_RE.fullmatch(value):
            raise CanonicalHashError(
                f"datetime string must use fixed millisecond UTC at {_format_path(path)}"
            )
...
def _format_datetime(value: datetime, *, path: tuple[str, ...]) -> str:
    ...
    if value.microsecond % 1000 != 0:
        raise CanonicalHashError(
            f"datetime must have fixed millisecond precision at {_format_path(path)}"
        )
