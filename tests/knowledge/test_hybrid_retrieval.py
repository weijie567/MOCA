...
async def _bind_character_corpus(session: AsyncSession, *, tenant_id) -> None:
    documents = list(
        (
    ...
    for sequence, document in enumerate(documents, start=1):
        ...
        immutable_document, immutable_chunks = await evidence.append_immutable_version(
            tenant_id=tenant_id,
            document=document,
            chunks=chunks,
    ...
    corpus = PolicyCorpusVersion(
        ...
        owner_marker="test.hybrid.fixture",
        run_token=None,
        ...
        validation_proof_json={"fixture": "hybrid_character_authority"},
...
async def test_sparse_repository_matches_chinese_domain_terms_in_postgres(
    ...
    await session.flush()
    await _bind_character_corpus(session, tenant_id=tenant_id)
    await _bind_character_corpus(session, tenant_id=other_tenant_id)
