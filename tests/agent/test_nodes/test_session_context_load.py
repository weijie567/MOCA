...
async def test_session_context_load_disabled_returns_empty_fallback(monkeypatch):
    monkeypatch.setattr(session_context_load_module.settings, "session_memory_enabled", False)

    result = await session_context_load(_state(), {"configurable": {"session": object()}})
...
async def test_session_context_load_without_async_session_returns_empty_fallback(monkeypatch):
...
async def test_session_context_load_without_bundle_returns_empty_fallback(monkeypatch):
    monkeypatch.setattr(session_context_load_module.settings, "session_memory_enabled", True)
...
async def test_session_context_load_attaches_session_memory_bundle(monkeypatch):
    monkeypatch.setattr(session_context_load_module.settings, "session_memory_enabled", True)
    run_id = str(uuid.uuid4())
...
async def test_session_context_load_fails_closed_when_bundle_load_fails(monkeypatch):
    monkeypatch.setattr(session_context_load_module.settings, "session_memory_enabled", True)
    run_id = str(uuid.uuid4())
    ...
    monkeypatch.setattr(session_context_load_module, "MemoryService", FakeMemoryService)
    monkeypatch.setattr(session_context_load_module, "SessionMemoryBundleService", FailingBundleService)
...
async def test_session_context_load_service_error_returns_unavailable(monkeypatch):
    monkeypatch.setattr(session_context_load_module.settings, "session_memory_enabled", True)
    run_id = str(uuid.uuid4())
