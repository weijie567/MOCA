...
- 以后在 MOCA 本地调试、启动、验证、UI 手测、API 测试、RAG/agent/记忆/工具调用排查中，只要发现错误、异常、不符合预期的回答、环境坑或验证失败，就要在处理后追加到 `.planning/LOCAL-VALIDATION-ISSUES.md`。记录默认使用中文，应包含：问题现象、如何检测/复现、关键证据或命令、当前判断/根因、已做处理、剩余问题和下次继续排查入口。`CLAUDE.md` 只保留这条记录规则，不写具体事故详情。

## 架构债务台账记录规则

- 修改**工具调用 / RAG / 记忆 / 意图识别**这几个核心子系统时，只要检出子系统级的 bug、设计缺陷、遗留妥协，或完成了对应修复，就**默认追加到 `.planning/ARCHITECTURE-DEBT.md`** 对应子系统章节。记录默认使用中文。
- 与 `.planning/LOCAL-VALIDATION-ISSUES.md` 的分工：后者记「本地调试/启动/验证时踩到的具体事故」，颗粒度细、生命周期短；`ARCHITECTURE-DEBT.md` 记「子系统级架构缺陷与处理台账」，颗粒度粗、生命周期长。二者可交叉引用，不重复正文。
- 每条目尽量给：问题现象/根因、影响、处理状态（✅已修复验证 / ⚠️修复但验证有缺口 / 🟡有意妥协 / 🔴待立项）、证据（phase / commit / 文件:行）、剩余风险。
- 只写「基于仓库真实代码、测试、planning artifact 核对过」的内容；未核实的显式标「未确认」，不编。目标态（`docs/contract-spec.md`）与已实现事实要分清。
- `CLAUDE.md` 只保留这条记录规则，不写具体缺陷详情。
